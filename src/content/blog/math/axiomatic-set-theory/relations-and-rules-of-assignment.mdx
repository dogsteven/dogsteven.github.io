...
<Corollary identifier="2">
  ...
  2. $F^{-1}\circ F = \text{id}_{\text{dom}(F)}$, với mọi $x\in \text{dom}(F)$ ta có $F^{-1}(F(x)) = x$.
  3. $F\circ F^{-1} = \text{id}_{\text{rng}(F)}$, với mọi $y\in \text{rng}(F)$ ta có $F\left(F^{-1}(y)\right) = y$.
</Corollary>
