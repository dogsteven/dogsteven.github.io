...
Theo [tiên đề 1](#axiom-1), tập hợp $p$ trong tiên đề trên là xác định duy nhất bởi $x$, ta gọi tập hợp này là **tập lũy thừa** (power set) của $x$ và lý hiệu cho nó bởi $\mathcal{P}(x)$.
...
<Theorem identifier="8" name="Inclusion ordering">
  Với mọi tập hợp $x, y$ và $z$, ta có:
  1. $x\subseteq x$.
  2. $x\subseteq y\subseteq x$ suy ra $x = y$.
  3. $x\subseteq y\subseteq z$ suy ra $x\subseteq z$.
</Theorem>
